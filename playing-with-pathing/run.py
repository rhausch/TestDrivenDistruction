...
def try_move_bug_simple(robot, target):
    ...
    if robot.id in bugging:
        ...
        if gc.can_move(robot.id, last_dir):
            ...
            this_dir = test_dir
            bugging[robot.id] = this_dir
            # print("BUG: Found a way out ", this_dir)
            return try_move_strict(robot, this_dir)
...
while True:
    ...
    try:
        ...
        for knight in myKnights:
            # print("Starting knight ", knight.id)
            sys.stdout.flush()
            ...
            if location.is_on_map():
                ...
                if target is not None:
                    ...
                    try_move_bug_simple(knight, target.location.map_location())
                    # print("Moved towards target")
                else:
                    # No target so move randomly
                    d = random.choice(directions)
                    if gc.is_move_ready(knight.id) and gc.can_move(knight.id, d):
                        gc.move_robot(knight.id, d)
            # print("Finished knight ", knight.id)
